 ...
 */
server.registerTool('compare-players-by-name', {
    ...
    try {
        ...
        let player1Candidates = [];
        let player2Candidates = [];
        if (league === 'mlb') {
            player1Candidates = player1Results.people || [];
            player2Candidates = player2Results.people || [];
        }
        else {
        ...
        if (activeStatus === 'Both' && (player1Candidates.length === 0 || player2Candidates.length === 0)) {
            if (player1Candidates.length === 0) {
                ...
                const inactiveCandidates1 = league === 'mlb' ? inactiveResults1.people || [] : Array.isArray(inactiveResults1) ? inactiveResults1 : [];
            ...
            if (player2Candidates.length === 0) {
                ...
                const inactiveCandidates2 = league === 'mlb' ? inactiveResults2.people || [] : Array.isArray(inactiveResults2) ? inactiveResults2 : [];
        ...
        if (searchResults.player1.found && searchResults.player2.found) {
            try {
                ...
                if (league === 'mlb') {
                    ...
                    const comparisonFactory = ComparisonFactory.getComparison(league);
                    if (league === 'nfl') {
            ...
            if (!searchResults.player1.found)
                missingPlayers.push(player1Name);
            if (!searchResults.player2.found)
                missingPlayers.push(player2Name);
        ...
        const formattedText = `Player Comparison by Name (${league.toUpperCase()})\n\n` +
            ...
            (comparison ?
                `Comparison Result:\n` +
                    ...
                    `${comparison.summary}\n\n` +
                    `Top Metrics:\n` +
                    comparison.metrics.slice(0, 5).map((metric, i) => `${i + 1}. ${metric.category}: ${metric.player1Value} vs ${metric.player2Value} (Winner: ${metric.winner === 'player1' ? comparison.player1.name : metric.winner === 'player2' ? comparison.player2.name : 'TIE'})`).join('\n')
                : error ?
                    `Error: ${error}`
