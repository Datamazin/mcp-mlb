...
function toNiceDate(yyyymmdd) {
    return `${yyyymmdd.slice(0, 4)}-${yyyymmdd.slice(4, 6)}-${yyyymmdd.slice(6, 8)}`;
}
export function registerGetTodaysNFLHighlightsTool(server) {
    server.registerTool('get-todays-nfl-highlights', {
        title: 'Get Today\'s NFL Highlights (ESPN)',
        description: 'Fetches ESPN NFL highlights for a date (defaults to today): per-game top fantasy players (PPR), leaders, and scoring plays.',
        inputSchema: {
            date: z
                .string()
                .optional()
...
//# sourceMappingURL=get-todays-nfl-highlights.js.map