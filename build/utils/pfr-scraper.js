...
export class PFRScraper {
     ...
     */
    async getPlayerStats(playerUrl, season) {
        try {
     ...
     */
    parsePlayerSearchResults(html, query) {
        const players = [];
        try {
    ...
    parsePlayerStats(html, season) {
        try {
            ...
            if (passingStatsMatch) {
                ...
                if (passingYardsMatch)
                    stats.passingYards = parseInt(passingYardsMatch[1]);
                if (passingTDsMatch)
                    stats.passingTDs = parseInt(passingTDsMatch[1]);
                if (interceptionsMatch)
                    stats.interceptions = parseInt(interceptionsMatch[1]);
            }
            return stats;
        }
        catch (error) {
     ...
     */
    isRelevantPlayer(playerName, query) {
...
//# sourceMappingURL=pfr-scraper.js.map