...
export class NFLWebScraper {
     ...
     */
    async getWeeklyStats(week, season) {
        const sources = [
     ...
     */
    async scrapeNFLComWeekly(week, season) {
     ...
     */
    async scrapePFRHistorical(playerName) {
     ...
     */
    async scrapeTeamOfficialSite(teamName) {
     ...
     */
    calculateConfidence(player, sources) {
        let confidence = 0;
    ...
    parseNFLComStatRow($row) { }
    extractPlayerUrlFromPFRSearch(html, playerName) { }
    parsePFRPlayerStats($) { }
    mergeStats(stats1, stats2) { }
}
//# sourceMappingURL=nfl-web-scraper.js.map