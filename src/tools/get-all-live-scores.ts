...
class RealMLBScraper {
    private mlbClient: any;
    
    constructor(mlbClient: any) {
    ...
    async getLiveGames() {
        try {
            ...
            return [{
                ...
                venue: 'API Error',
                attendance: 0,
                situation: (error as Error).message
            }];
        }
    }
    ...
    private getGameSituation(game: any): string {
        if (!game.linescore) return 'Pregame';
        ...
        if (game.status?.detailedState === 'In Progress') {
            ...
            if (game.linescore.offense) {
                const runners = [];
                if (game.linescore.offense.first) runners.push('1st');
                if (game.linescore.offense.second) runners.push('2nd');
                if (game.linescore.offense.third) runners.push('3rd');
...
export function registerGetAllLiveScoresTool(server: any, mlbClient?: any) {
            ...
            try {
                ...
                for (const sport of sports) {
                    try {
                        ...
                        results.sports[sport] = { error: (error as Error).message };
                    }
                }
...
function formatCompact(results: any) {
    const compact = {
    ...
    Object.entries(results.sports).forEach(([sport, data]: [string, any]) => {
        if (data.games) {
...
function formatDetailed(results: any) {
