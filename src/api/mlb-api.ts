...
export class MLBAPIClient extends BaseSportAPI {
  ...
  async getPlayerStats(
    ...
    const primaryStats = data.stats.find((stat: any) => 
      stat.group && stat.group.displayName && stat.splits && stat.splits.length > 0
    );
   ...
   */
  async searchMLBPlayers(name: string, activeStatus: string = 'Y'): Promise<any> {
    const currentYear = new Date().getFullYear();
    const params = {
      sportId: 1,
      season: currentYear,
   ...
   */
  async getBoxscore(gamePk: number, timecode?: string): Promise<any> {
    const params: Record<string, any> = {};
    if (timecode) params.timecode = timecode;
    ...
    const processTeamBoxscore = (teamData: any) => {
      const players = [];
    ...
    return {
      gamePk,
      gameInfo: {
        attendance: data.info?.find((info: any) => info.label === 'Attendance')?.value,
        weather: data.info?.find((info: any) => info.label === 'Weather')?.value,
        wind: data.info?.find((info: any) => info.label === 'Wind')?.value,
        firstPitch: data.info?.find((info: any) => info.label === 'First pitch')?.value,
        timeOfGame: data.info?.find((info: any) => info.label === 'T')?.value
      },
      teams: {
   ...
   */
  async getJobs(jobType: string, sportId: number = 1, date?: string): Promise<any> {
    const params: Record<string, any> = {
      jobType: jobType
    };
   ...
   */
  async getMeta(type: string, ver: string = 'v1'): Promise<any> {
   ...
   */
  async searchPlayers(name: string, options?: any): Promise<BasePlayer[]> {
    ...
    try {
      const results: any = await this.searchMLBPlayers(name, options?.activeStatus || 'Y');
      return results.people?.map((player: any) => ({
   ...
   */
  async getTeams(options?: any): Promise<BaseTeam[]> {
