...
import { NBAPlayerStats, UniversalPlayer, LiveGame, InjuryReport, WebScrapingConfig } from '../types/multi-sport-types.js';
...
export class NBAWebScraper {
   ...
   */
  async getPlayerStats(playerName: string, season?: number): Promise<NBAPlayerStats | null> {
  ...
  private async scrapeNBAComPlayer(playerName: string, season?: number): Promise<any> {
  ...
  private async scrapeBasketballReference(playerName: string, season?: number): Promise<any> {
  ...
  private async scrapeESPNStats(playerName: string, season?: number): Promise<any> {
  ...
  private async searchNBACom(query: string): Promise<any> {
    // Implementation for NBA.com search
    return [];
  }

  private async searchBasketballReference(query: string): Promise<any> {
  ...
  private consolidateNBAStats(results: any[]): NBAPlayerStats {
    const consolidated: NBAPlayerStats = {
      regular: {},
      playoffs: {},
      advanced: {}
    };
  ...
  private parseQuarter(quarterText: string): number | undefined {
  ...
  private assessInjuryImpact(status: string): 'Minor' | 'Moderate' | 'Severe' {
    const normalized = status.toLowerCase();
    if (normalized.includes('out')) return 'Severe';
    if (normalized.includes('doubtful')) return 'Moderate';
    if (normalized.includes('questionable')) return 'Minor';
    if (normalized.includes('probable')) return 'Minor';
    return 'Moderate';
  }

  private extractPlayerUrlFromBBRef(html: string, playerName: string): string | null {
  ...
  private parseBasketballReferenceProfile($: cheerio.Root): any {
    return {
  ...
  private parseBasketballReferenceStats($: cheerio.Root): any {
    const stats = {
      regular: {},
      playoffs: {},
      advanced: {}
